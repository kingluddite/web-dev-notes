...
* The problem is when Waypoints makes that measurement (say Testimonials is 6000px from the top), lazysizes hasn't loaded our images yet and that 6000px measurement is outdated the moment those images appear
...
* We don't need to repeat this code again in `RevealOnScroll.js` because of how the Webpoint library is made
    ...
    - The Waypoint library just attaches an object named Waypoint to the browsers Global Window scope
        + So when we call it above `Waypoint.refreshAll()` we are calling it through the Window scope we are refreshing all Waypoints that exist in the browsers memory
