...
## TODO
* [0-60 with typescript and node.js](https://www.youtube.com/watch?v=vxvQPHFJDRo)
...
## Copying favicon stuff to dist/public from src/public on build
```
...
## Troubleshooting
So this has nothing to do with Typescript. The problem is that you are using fat arrow syntax to define mongoose methods and hooks. Don't do that. If you use a fat arrow function to define a pre save hook, it has no lexical scope and cannot be bound to a given context.

## Model code :

```
schema.methods.authenticate = async function(password: string) { return await bcrypt.compareSync(password, this.password); };
```

## Router code :
```
if (await (user as any).authenticate(password)) { res.json({ message: "SUCCESS LOGIN" }); }
("user" is what return from the db from findOne method)
```
