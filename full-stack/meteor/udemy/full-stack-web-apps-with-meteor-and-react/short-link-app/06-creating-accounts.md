...
* This will enable password-based signups and logins
...
What it is saying is that we are using a JavaScript implementation of the **bcrypt** algorithm (_that is a hashing algorithm that secures your password_) and this will work but it is slower than a native implementation. You could run that command to switch but it is recommended to keep a pure JavaScript version as it tends to be more stable in cross-operating environments

**note** Even though we install `accounts-password` we never import it. This package installs dependencies and makes changes to some of those modules adding new password related functionality
...
This will give us a good idea of what the `createUser()` method is doing (_**note** `createUser()` will create a user and automatically sign them in with the new account - which is great, you sign up on the form and you go right into the app - there is no need to signup and then go back to the login screen and type your the same login information again_)
...
* You will see [salted and hashed password](https://crackstation.net/hashing-security.htm) which means we are never revealing the plain text password which is important for good security
...
Finally we get the `result`, lets us know all is great and we get back some user information like tokens (_our authentication token_) and we have info when that token expires
...
With `import` we can load in **defaults** or **named exports** but with `require` we load in a file and it returns an object with everything inside it
...
So if a user visits our app and they have an `id` we can redirect them to a private page, if not we'll leave them on the login page, making them log in
...
Now `> require('meteor/meteor').Meteor.userId()` returns **null** which lets us know there is no logged in user and `> require('meteor/meteor').Meteor.user()` also returns **null**
...
We will check out the users Collection that we did not create nor will we directly manage

Meteor created it and will maintain it for us

All the updates and deletions happen behind the scenes. We just call the methods provided and Meteor takes care of that stuff for us
